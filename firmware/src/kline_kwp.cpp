...
// 16 bytes comfortably covers every current request/response (largest is
// an 8-byte 2-payload-byte Mode 01 response: 3-byte header + 2 SID/PID +
// 2 payload + 1 checksum), well inside kline_frame.h's protocol-max
// kKlineMaxDataLen (63). Revisit if a later phase adds a request needing
// a payload anywhere near that bound.
